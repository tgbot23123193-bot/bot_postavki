...
class RedistributionConfig:
    ...
    def get_booking_periods() -> List[Tuple[time, time]]:
        """
        Получает периоды для попыток бронирования из ENV.
    ...
    def get_retry_minutes() -> int:
        """Получает время ожидания между попытками в минутах."""
    ...
    def get_max_attempts() -> int:
        ...
        return int(os.getenv('REDISTRIBUTION_MAX_ATTEMPTS', '10'))
    
    @staticmethod
    def is_in_booking_period() -> bool:
        """
        Проверяет, находимся ли мы в одном из периодов бронирования.
        
        Returns:
            bool: True если в периоде, False если нет
        """
    ...
    def minutes_until_next_period() -> int:
        """
        Возвращает количество минут до следующего периода бронирования.
