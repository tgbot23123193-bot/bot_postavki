...
class User(Base):
    ...
    extension_link: Optional["ExtensionLink"] = relationship(
        "ExtensionLink",
        back_populates="user",
        cascade="all, delete-orphan",
        uselist=False
...
class ExtensionLink(Base):
    """Extension link model for linking Chrome extension to bot."""
    ...
    # Foreign key to user
    user_id: int = Column(BigInteger, ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True, unique=True)
    
    # Link key (generated by bot)
    link_key: str = Column(String(100), nullable=False, unique=True, index=True)
    
    # Status
    is_active: bool = Column(Boolean, default=True, nullable=False, index=True)
    
    # Extension info
    extension_version: Optional[str] = Column(String(20), nullable=True)
    last_activity: Optional[datetime] = Column(DateTime, nullable=True, index=True)
    
    # Timestamps
    created_at: datetime = Column(DateTime, default=func.now(), nullable=False, index=True)
    linked_at: Optional[datetime] = Column(DateTime, nullable=True, index=True)
    
    # Relationship
    user: "User" = relationship("User", back_populates="extension_link")
    
    # Indexes
    __table_args__ = (
        Index('idx_extension_links_user_id', 'user_id'),
        Index('idx_extension_links_link_key', 'link_key'),
        Index('idx_extension_links_active', 'is_active'),
    )
    
    def __repr__(self):
        return f"<ExtensionLink(id={self.id}, user_id={self.user_id}, is_active={self.is_active})>"
