...
async def show_monitoring_menu(callback: CallbackQuery):
    ...
    # Если есть API ключи
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📊 Мониторинг доступен!", callback_data="monitoring_available")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_main")]
    ])
    
    await callback.message.edit_text(
        "📊 <b>Мониторинг слотов</b>\n\n"
        "✅ Отлично! API ключи настроены.\n"
        "Теперь вы можете использовать мониторинг слотов.\n\n"
        "Функция мониторинга пока в разработке...",
        parse_mode="HTML",
        reply_markup=keyboard
    )
    await callback.answer()
...
async def start_add_api_key(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state("waiting_api_key")
...
async def handle_api_key_input(message: Message, state: FSMContext):
    ...
    if current_state == "waiting_api_key":
